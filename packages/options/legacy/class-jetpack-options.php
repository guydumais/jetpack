...
class Jetpack_Options {
	...
	public static function delete_all_known_options() {
		 ...
		 * @since 8.7.0
		 */
		$untouchables = array_filter( apply_filters( 'jetpack_options_delete_all_ignore', array() ), 'is_array' );

		// Delete the compact options.
		foreach ( (array) self::$grouped_options as $group => $option_name ) {
			if ( empty( $untouchables[ $group ] ) ) {
				// Delete all compact options.
				delete_option( $option_name );
				continue;
			}

			self::delete_grouped_option( $group, array_diff( self::get_option_names( $group ), $untouchables[ $group ] ) );
		}

		// Delete all non-compact Jetpack options.
		foreach ( (array) self::get_option_names( 'non-compact' ) as $option_name ) {
			if ( empty( $untouchables['non-compact'] ) || ! in_array( $option_name, $untouchables['non-compact'], true ) ) {
